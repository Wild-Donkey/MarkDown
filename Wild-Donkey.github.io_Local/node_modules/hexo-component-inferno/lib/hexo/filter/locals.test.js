...
test('Page/post layout configs override theme and site configs', async () => {
  ...
  const configs = [Object.assign({}, basePageConfig, {
    layout: 'page'
  }), Object.assign({}, basePageConfig, {
    layout: 'post'
  })];

  for (const page of configs) {
    const locals = await hexo.extend.filter.exec('template_locals', new Locals('about/index.html', page), {
      context: hexo
    });
    expect(locals.config.comments).toBeUndefined(); // theme page/post layout configs override ones under site root
...
test('Configs in page/post front-matter override all other config sources', async () => {
  ...
  for (const page of configs) {
    ...
    ['int', 'float', 'bool', 'str', 'empty', 'arr', 'obj', 'int_ex', 'float_ex', 'bool_ex', 'str_ex', 'empty_ex', 'arr_ex', 'obj_ex'].forEach(key => {
