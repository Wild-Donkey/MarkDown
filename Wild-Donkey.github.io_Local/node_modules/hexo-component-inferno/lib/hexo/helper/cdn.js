...
const CDNJS_FIXTURES = {
  ...
  outdatedbrowser: (ver, fname) => ['outdated-browser', ver, fname.startsWith('outdatedbrowser/') ? fname.substr(16) : fname],
  'highlight.js': (ver, fname) => ['highlight.js', ver, fname.endsWith('.css') && fname.indexOf('.min.') === -1 ? fname.substr(0, fname.length - 4) + '.min.css' : fname],
  mathjax: (ver, fname) => ['mathjax', ver, (() => {
  ...
  disqusjs: (ver, fname) => [],
  cookieconsent: (ver, fname) => ['cookieconsent', ver, fname.startsWith('build/') ? fname.substr(6) : fname],
  '@waline/client': (ver, fname) => ['waline', ver, fname]
};
const UNPKG_FIXTURES = {
  mathjax: (ver, fname) => ['mathjax', ver, fname.indexOf('.min.') > -1 ? fname.replace(/\.min\./gi, '.') : fname],
  disqusjs: (ver, fname) => ['disqusjs', ver, fname.indexOf('.min.') > -1 ? fname.replace(/\.min\./gi, '.') : fname]
};
/**
 * Register the Hexo helper functions for resolving the URL of static files of JavaScript libraries,
 * web fonts, and FontAwesome icon fonts.
...
module.exports = function (hexo) {
  function applyFixture(fixture, _package, version, filename) {
    if (Object.prototype.hasOwnProperty.call(fixture, _package)) {
      const result = fixture[_package](version, filename); // package is not hosted on the given CDN
  ...
  hexo.extend.helper.register('cdn', function (_package, version, filename) {
    ...
    return _cdn.replace(/\${\s*package\s*}/gi, _package).replace(/\${\s*version\s*}/gi, version).replace(/\${\s*filename\s*}/gi, filename);
  });
