...
class Migrator {
  ...
  migrate(config, toVersion = null, callback = null) {
    ...
    const tVer = toVersion || this.getLatestVersion(); // find all migrations whose version is larger than fromVer, smaller or equal to toVer
    // and run migrations on the config one by one.

    return this.versions.filter(ver => semver.gt(ver, fVer) && !semver.gt(ver, tVer)).sort(semver.compare).reduce((cfg, ver) => {
