...
describe('DefaultValue class tests', () => {
  describe('#clone()', () => {
    ...
    test('Clone nested DefaultValue', () => {
      const source = new DefaultValue(new DefaultValue({
        a: 1
      }, 'child'), 'parent');
  ...
  describe('#flatten()', () => {
    ...
    test("Don't flatten DefaultValue is its value is not a DefaultValue", () => {
      ...
      expect(source.value.value.value).toEqual({
        a: 1
      });
    });
  });
  describe('#merge()', () => {
    ...
    test('Recursive merge if .value is a DefaultValue', () => {
      const target = new DefaultValue(new DefaultValue({
        a: 1
      }));
      const source = new DefaultValue({
        b: 2
  ...
  describe('#toCommented()', () => {
    ...
    test('#toCommentedArray()', () => {
      const source = new DefaultValue([0, new DefaultValue(1, 'child-1'), 2, new DefaultValue(3, 'child-3\nchild-4'), new DefaultValue(4)], 'parent');
      expect(source.toCommented()).toEqual(source.toCommentedArray());
      expect(source.toCommented()).toEqual([0, MAGIC + '0: child-1', 1, 2, MAGIC + '0: child-3', MAGIC + '1: child-4', 3, 4]);
    });
    ...
    test('Complex cases', () => {
      ...
      expect(source.toCommented()).toEqual({
        a: [0, MAGIC + '0: child-1', 1]
      });
      source = new DefaultValue([0, {
        a: new DefaultValue(1, 'child-1')
      }], 'parent');
      expect(source.toCommented()).toEqual([0, {
        [MAGIC + 'a0']: 'child-1',
        a: 1
      }]);
    });
...
describe('SchemaLoader class tests', () => {
  ...
  const definitions = [require('../../fixture/schema/root'), require('../../fixture/schema/parent'), require('../../fixture/schema/child1'), require('../../fixture/schema/child2')];
  test('#addSchema()', () => {
  ...
  test('#removeSchema()', () => {
    ...
    expect(Object.prototype.hasOwnProperty.call(loader, definitions[definitions.length - 1].$id)).toBe(false);
    loader.addSchema(definitions[0]);
  ...
  test('SchemaLoader.load()', () => {
    ...
    expect(loader.getSchema('/comment/disqus.json').def).toEqual(require('../schema/comment/disqus.json'));
    expect(() => {
