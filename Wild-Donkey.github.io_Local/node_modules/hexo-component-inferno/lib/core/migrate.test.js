...
test('Do full upgrade correctly', () => {
  ...
  const result = migrator.migrate({
    version: '1.0.0'
  }, null, callback); // callback has been invoked twice

  expect(callback.mock.calls.length).toBe(2); // first invocation received the correct version parameters

  expect(callback.mock.calls[0][0]).toBe('1.0.0');
  expect(callback.mock.calls[0][1]).toBe('2.0.0'); // second invocation received the correct version parameters
...
test('Do partial upgrade correctly', () => {
  ...
  const result = migrator.migrate({
    version: '1.0.0'
  }, '2.1.0', callback); // callback has been invoked twice

  expect(callback.mock.calls.length).toBe(1); // first invocation received the correct version parameters
