...
### [LOJ6036](https://loj.ac/p/6036) [P6965](https://www.luogu.com.cn/problem/P6965)
...
建 Trie, 每个字符串插入, 分别保存 `?` 取 `0` 和 `1` 的情况对应的结尾节点, 一个节点选择后, 它子树上的节点都不能选, 它到根的路径上的节点也不能选, 所以需要向子树和.
...
struct Node {
  vector<Node*> To;
  Trie* Tr;
  unsigned Bel, Low, DFSr, Num, Pos;
  char InS;
}N[3000005], *List[3000005], * CntN, * Frm;
struct Trie {
  Trie* Son[2], * Top, * Fa;
...
struct Seg {
  ...
  inline void Link(unsigned L, unsigned R) {
    if((A <= L) && (R <= B)) {Frm->To.emplace_back(Pnt);return;}
    unsigned Mid((L + R) >> 1);
    if(A <= Mid) LS->Link(L, Mid);
    if(B > Mid) RS->Link(Mid + 1, R);
  }
}S[2000005], *CntS(S);
inline void Build(Seg* x, unsigned L, unsigned R) {
  if(L == R) {x->Pnt = N + ((List[L] - N) ^ 1); return;}
  x->Pnt = ++CntN;
  unsigned Mid((L + R) >> 1);
  Build(x->LS = ++CntS, L, Mid);
  Build(x->RS = ++CntS, Mid + 1, R);
  x->Pnt->To.emplace_back(x->LS->Pnt);
  x->Pnt->To.emplace_back(x->RS->Pnt);
}
inline void Tarjan(Node* x) {
