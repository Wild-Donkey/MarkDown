...
$$
\min_{k = RK_{A_i} + 1}^{RK_{B_{j + 1}}} Height_{k}
= \min (\min_{k = RK_{A_i} + 1}^{RK_{B_j}} Height_{k}, \min_{k = RK_{B_j} + 1}^{RK_{B_{j + 1}}} Height_{k})
$$
...
二分答案, 设 LCP 为 $x$. 这样就可以倍增找到点 $[1, c + x - 1]$ 的后缀链接中深度最小的 $Len \geq x$ 的点 $X$. 然后查询它的
...
### [ZJOI2015](https://www.luogu.com.cn/problem/P3345)
