...
问题分为: (计算)函数问题和决策问题. (分类)决策问题形如: 输入是否是 A, 可被编码为字符串集合, 即语言.
...
图灵可识别语言的子集, 存在一定能停机的图灵机接受的语言. 一个计算问题是可判定的, 也说这个问题是可计算的.

显然一个可判定语言的补也是可判定的. (对输出取反)

## 不可判定语言

对图灵可识别语言 $L$ 取补集, 如果 $\overline L$ 是图灵不可识别的, 那么根据可判定语言的补集也可判定, $L$ 一定是不可判定的.

使用单个字符的字符集 $\{a\}$. 显然字符串可数, 图灵机可数, 而语言不可数. 将图灵机按从上到下, 字符串从左到右, 构造一个接受矩阵. 使用对角线法则, 将对角线的值作为 $L$, 取反得到一个图灵不接受语言 $\overline L$, 因此如果 $L$ 是图灵接受, 那么它是不可判定语言. $L$ 是图灵接受的, 构造图灵机: 对于一个输入 $a^i$, 用枚举器枚举到接受第 $i$ 个语言的图灵机, 然后模拟这个图灵机运行 $a^i$, 如果接受则接受.
...
如果问题 x 能归约到问题 y, 只要能解决 y, 就能解决 x. 如果已知 x 不可解, 则 y 和 $\overline y$ 也不可解.

可计算函数 f: 对任意串 w, 存在图灵机可以计算 $f(w)$, 然后保存到纸带上.

语言 A 归约到语言 B: 存在可计算函数 f, 使得对任意串 w, $w \in A \iff f(w) \in B$.

## 不可计算问题

成员问题: 给一个图灵机和一个串, 判定该图灵机是否接受该串. 如果这个问题可判定, 则所有图灵可接受语言都是可判定的, 矛盾. 

停机问题: 判定程序在给定输入下是否停机. 如果可判定, 则所有图灵可接受语言都是可判定的, 如果不停机则拒绝, 否则模拟原来的图灵机即可. 另一个证明是假设可判定, 存在判定器 H, 构造 H', 在 H 判定停机后进入循环, 否则停机. 构造 F, 输入一个图灵机, 然后将图灵机既作为图灵机也作为串传给 H', 然后把 F 自身传入 F, 如果 F 停机, 则 H' 中会死循环, F 事实上不停机; 如果 F 不停机, 则 H' 会判定为不停机, 于是停机. 两种情况都矛盾. 

进入状态问题: 判定图灵机在给定输入下是否进入某个特定状态. 将停机问题规约道该问题, 可证明该问题不可解. 只需令所有接受状态都连到一个新状态, 然后判定该新状态是否被进入即可.

空带停机问题: 判定图灵机在空带输入下是否停机. 构造图灵机将输入串写入纸带, 然后将整个系统输入空带停机问题. 这样就将停机问题规约到空带停机问题.

空语言问题: 输入图灵机, 判定它接受的语言是否为空. 可以由成员问题规约. 构造图灵机, 先判断输入是否等于 $w$, 如果不是, 拒绝, 否则模拟 $w$ 在原图灵机上的运行. 如果原图灵机接受 $w$, 则新图灵机接受 $w$, 否则新图灵机接受空语言.

正则语言问题: 判断输入的图灵机接受的语言是否是正则语言. 相似, 规约自成员问题. 构造图灵机, 先构造某个非正则语言的判定器 (比如 $\{a^nb^n\}$), 判断输入是否属于该语言. 如果是, 则模拟原图灵机判断 $w$, 否则拒绝. 返回的图灵机进入正则语言问题判断器, 如果原图灵机接受 $w$, 则输入判断器的图灵机接受非正则语言 $\{a^nb^n\}$, 否则接受空语言, 也就是正则语言. 

大小为 $2$ 的语言问题: 判断输入的图灵机接受的语言是否有且只有两个字符串. 依然是相似的, 规约自成员问题. 思想还是构造两个接受条件的图灵机: 输入字符串在某两个字符串的语言中且原图灵机接受 $w$, 则接受; 否则拒绝. 然后输入到大小为 $2$ 的语言问题判断器即可.
...
考虑一个图灵机 M, 它判定语言 L, 对任意输入在有限步停机, 步数也就是时间. 输入 $n$ 长度的串, 耗时最久的记为 $T_M(n)$ 步, 则 $T_M$ 的取值构成一个函数. 用大 $O$ 符号表示函数随 $n$ 的增长趋势.

时间复杂度类: 所有在某个时间复杂度函数 $O(T(n))$ 内可判定语言的集合, 记为 TIME($T(n)$).

TIME($n$): 线性时间可判定语言集合. $\{a^nb\}$, $\{b^n: n \% 2 = 0\}$.

TIME($n^2$): $\{a^nb^n\}$, $\{ww^R\}$, $\{ww\}$.

TIME($n^3$): CYK Algorithm (判断串 w 是否由上下文无关文法 G 派生), 矩阵乘法.

TIME($n^k$): 多项式时间可判定语言集合. 记为 P. 包含所有正则语言和上下文无关语言. TIME($n^k$) $\subset$ TIME($n^{k + 1}$).
