...
* 第 $5-8$ 周
数据结构进阶, 包括数据结构持久化, 轻重链链剖分, LCT, 边带权/扩展域并查集, CDQ 分治, 莫队算法.
...
附加 `C++` 语法相关 (容器, 迭代器, 空间配置器, 配接器, 算法, 仿函数, 面向对象)
...
* 七月
进行数据结构 + 字符串 + 搜索的学习. 内容包括: 左偏树(可并堆), 斐波那契堆, 二项堆,  长链剖分, KD-Tree, 树套树, 莫队进阶, 红黑树, AVL, 宗法树, FHQ, SBT, 珂朵莉树, zkw线段树, 笛卡尔树, 李超线段树, 树上启发式合并, 点分治, 析合树, 哈夫曼树, 二维线段树; 序列自动机, 最小表示, Lyndon 分解, EXKMP, Boyer-Moore 算法, DFA, NFA; Dancing Links; 跳跃表. 

* 八月
DP + 图论, 一般图匹配, 二分图 KM, 上下界最大流, 费用流, ISAP/当前弧优化的Dinic, 支配集, 独立集, 覆盖集, 斯坦纳树, 虚树, 矩阵树定理, 最小树形图, 生成树相关(次小生成树, 最短路径生成树, 最小直径生成树, 最优比率生成树), 最短路相关(乘积最短路, 加和最短路, 同余最短路), 圆方树, Prufer 序列, LGV 引理, 弦图; 动态动态规划 (DDP), 插头 DP, WQS 优化, CDQ 分治优化 (DP重在练习).

* 九月
数学, 熵, 多项式相关 (快速傅里叶变换(FFT), 快速数论变换(NTT), 快速沃尔什变换(FWT), 拉格朗日插值, 多项式除法, 多项式求逆, 多项式开根, 多项式求对数, 多项式快速幂, 多项式指数函数), 牛顿迭代, 生成函数(数学), 线性代数, 线性规划, 组合数学进阶 (可重集, 错/圆排列, 卡特兰数, 斯特林数), 狄利克雷卷积, 容斥原理, 博弈论(Nim, SG), 群论 (置换群/循环群, Burnside, Polya), 数列, 高等数学(泰勒级数, 卷积), 计算几何 (半平面交, 凸包, 旋转卡壳, 矢量), 数论(完全数, BSGS, 原根, 筛法(杜教筛, Min_25筛, 洲阁筛), 反演相关(多项式反演, 莫比乌斯反演等), 素数, 整除与同余), 威尔逊定理, 中国剩余定理 (CRT), 线性代数 (矩阵的逆运算, 行列式, 线性基, 线性方程组的高斯消元), 平方剩余, 二次同余式, 二次互反律, 母函数, 莫比乌斯变换, gAy率论(贝叶斯公式, 全概率公式, 乘法公式).
